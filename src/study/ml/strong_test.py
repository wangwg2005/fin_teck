...
url_rzrq='https://datacenter-web.eastmoney.com/api/data/get?type=RPTA_WEB_RZRQ_GGMX&sty=ALL&source=WEB&st=DATE&sr=-1&p=1&ps=240&filter=(scode%3D002118)&callback=jQuery112307362262706310092_1648979761578&_=1648979761579'
...
def get_feature(sid):
    ...
    print(r)
    data = map(lambda a:a[:6],r['data'][sid]['qfqday'])
    print(data)
    
    df = pd.DataFrame.from_records(data,columns=['day','open','close','high','low','volume'])
...
#    df = pd.read_csv(r'C:\Users\Darren\eclipse-workspace\fin_study\src\study\history\stock\002118\price.csv',encoding='utf8',index_col=[0],parse_dates=True)[-500:];
    ...
    df['voume'] = df['volume'].astype(float)
    df['high'] = df['high']
...
#     df=df.dropna()
    ...
    df['s']=df['pct'].map(lambda a: int(a*1000) if a>0 else - a*1000)
    df['pct_s']=df['pct'].shift()
    print(df.tail())
    s1 = mpf.make_mpf_style(base_mpl_style="ggplot", marketcolors=mc)
    df['avg']=df['close'].rolling(5).mean()
    add_plot=[mpf.make_addplot(df['vol_price'],panel=0,color='red'),
              #mpf.make_addplot(df['avg'],panel=0,color='green'),
              mpf.make_addplot(df['vol_volume']/100,panel=0,color='green')]
