...
#     dfs=filter(lambda df:len(df[1])>1000,dfs)
    ...
    result=pd.DataFrame(columns=["sid","diff","ratio"],dtype=object)
    ...
    for sid,df in dfs:
        ...
        last_obs=df["close"][-1]
        mse=(y_pred["mean"]/df["close"]-1)**2
        
        r={"sid":sid,"diff":last_pred-last_obs,"ratio":last_pred/last_obs-1,"mse":mse.mean(),"mse_median":mse.median()}
        result=result.append(r,ignore_index=True)
    
    result=result.set_index("sid")
    result=result.sort_values(by="mse",ascending=True)
    print(result)
