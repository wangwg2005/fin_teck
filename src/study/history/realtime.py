...
#     t1=[]
    ...
    y1=list(map(lambda a:float(a[4].replace(',','')),t1))
    if appendToday:
        y1.append(csiCurr)
...
#     print(y2)
    ...
    y2=list(map(lambda a:float(a[3]),t2))
    plt.plot(x,y2,'b')
     
     
    y7=list(map(lambda a:(y1[a]*2.7-1000-y2[a])*2,x))
    plt.plot(y7)
     
#     y8=list(map(lambda a:(hs300[a]*2.7+11000-y2[a])*2,x))
#     plt.plot(x,y8)
