...
#         add_plot.append(mpf.make_addplot(y_pred["mean"]))
        ...
        print("loop for train length = ", train_len)
        add_plot=[mpf.make_addplot(y_pred["mean"])]
        mpf.plot(dataset[-len(y_pred):],type="candle",volume=True,style=ds.get_style(),addplot=add_plot,title="sh000905",fill_between=filled,savefig=os.path.join("explore","sh000905_{0}_timeseries.png".format(train_len)))
        
    dataset = dataset[5*48:]
